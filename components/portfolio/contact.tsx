    ...
    <section id="contact" className="bg-background py-24 md:py-32">
      ...
      >
        <div className="max-w-xl mx-auto text-center mb-12">
          ...
          <p className="text-muted-foreground leading-relaxed text-pretty">{c.subtitle}</p>
        </div>

        <div className="max-w-lg mx-auto">
          {formState === "success" ? (
            <div className="text-center py-16 animate-fade-in-up">
              <div className="w-16 h-16 bg-emerald-light/50 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg aria-hidden="true" className="w-8 h-8 text-emerald" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <h3 className="text-lg font-semibold text-foreground mb-2">{c.successTitle}</h3>
              <p className="text-muted-foreground text-sm">{c.successText}</p>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 text-sm bg-card border border-border rounded-lg text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-emerald/30 focus:border-emerald transition-colors duration-200"
                />
              </div>
              <div>
              ...
              <button
                ...
                className={cn(
                  "w-full py-3 px-6 bg-emerald text-emerald-foreground font-medium text-sm rounded-lg transition-all duration-200",
                  formState === "submitting"
                    ? "opacity-70 cursor-not-allowed"
                    : "hover:bg-emerald/90 hover:shadow-sm"
                )}
              >
                {formState === "submitting" ? (
          ...
          <div className="flex items-center justify-center gap-4 mt-10 pt-8 border-t border-border">
            <a
              href="https://linkedin.com/in/lucaspereyradev/"
              target="_blank"
...
export function Footer() {
  ...
  return (
    <footer className="bg-surface border-t border-border">
      <div className="h-0.5 w-full bg-emerald/20">
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          {NAV_LABELS.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-xs text-muted-foreground hover:text-emerald transition-colors duration-200"
            >
              {item.label}
            </a>
          ))}
